...
type UseAlertsOptions = {
  /** When false, skip fetching until the screen is focused (default). */
  enabled?: boolean;
};

export function useAlerts(filter: AlertFilter = 'all', options?: UseAlertsOptions) {
  const isFocused = useIsFocused();
  const enabled = options?.enabled ?? isFocused;
  const { isAuthenticated } = useAuth();
  const { children, isLoading: guardianLoading } = useGuardianData();
  ...
  const refresh = useCallback(async () => {
    if (!isAuthenticated || guardianLoading) {
      if (!isAuthenticated) {
        setAlerts([]);
        setLoading(false);
      }
