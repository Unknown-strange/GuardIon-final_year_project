...
from app.database import run_with_db_retry_async
...
async def _process_geofence_alerts(
    ...
    """Run zone checks in a background thread; only one geofence job at a time."""
    ...
    async with _geofence_sem:
        try:
            alert_broadcasts = await run_with_db_retry_async(geofence_work)
    ...
    try:
        ...
        def save_work(db):
            ...
            if (
                ...
                and _should_insert_location_history(device_id)
            ):
                db.add(
                    LocationHistory(
                        device_id=device.id,
...
async def handle_alert(payload: Dict):
    ...
    try:
        ...
        try:
            result = await run_with_db_retry_async(db_work)
        except PoolTimeoutError:
            logger.error("Alert skipped for %s: database pool exhausted", device_id)
            return
        if result is None:
            return
...
async def handle_status(payload: Dict):
    ...
    try:
        ...
        try:
            await run_with_db_retry_async(db_work)
        except PoolTimeoutError:
            logger.error("Status skipped for %s: database pool exhausted", data.device_id)
    except Exception as e:
